...
// the APIServiceExport object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.20.4/pkg/reconcile
func (r *APIServiceExportReconciler) Reconcile(ctx context.Context, req mctrl.Request) (mctrl.Result, error) {
...
func (r *APIServiceExportReconciler) SetupWithManager(mgr mctrl.Manager) error {
	return mctrl.NewControllerManagedBy(mgr).
		For(&kubebindv1alpha1.APIServiceExport{}).
		Named("kube-bind-apiserviceexport").
		Complete(r)
}
