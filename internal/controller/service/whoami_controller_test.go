...
var _ = Describe("Whoami Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind Whoami")
			err := k8sClient.Get(ctx, typeNamespacedName, whoami)
			if err != nil && errors.IsNotFound(err) {
				resource := &servicev1alpha1.Whoami{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Whoami")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &WhoamiReconciler{}
